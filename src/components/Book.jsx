...
import { editedBook } from "../redux/books/actions";
import deleteBook from "../redux/books/thunk/deleteBook";
export default function Book({ book }) {
  const dispatch = useDispatch();
  const stars = Array.from({ length: book.rating }).fill(1);

  // delete a book
  const handleDeletedBook = (bookId) => {
    dispatch(deleteBook(bookId));
  };

  // edit a book
  const handleEditedBook = (bookId) => {
    dispatch(editedBook(bookId));
  };
  return (
      ...
      <div className="flex-1 h-full pr-2 pt-2 flex flex-col">
        <div className="flex items-center justify-between">
          ...
          <div className="text-gray-500 space-x-2">
            <button
              className="lws-edit"
              onClick={() => handleEditedBook(book.id)}
            >
              <svg
                fill="none"
                viewBox="0 0 24 24"
            ...
            <button
              ...
              onClick={() => handleDeletedBook(book.id)}
            >
              <svg
                fill="none"
                viewBox="0 0 24 24"
